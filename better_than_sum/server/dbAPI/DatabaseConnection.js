  ...
  async _postProduct(id, baseCost, name, owningUser) {
    if (isNaN(id)) {
      let result = await this._connection.awaitQuery(
        `INSERT INTO Products (baseCost, name, owningUser) VALUES (?, ?, ?);`,
        [baseCost, name, owningUser]
      );
      return JSON.stringify({ insertedID: result.insertId });
    } else {
      await this._connection.awaitQuery(
        `UPDATE Products SET baseCost = ?, name = ?, owningUser = ? WHERE productID = ?;`,
        [baseCost, name, owningUser, id]
      );
