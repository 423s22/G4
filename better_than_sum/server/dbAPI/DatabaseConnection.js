...
export default class DatabaseConnection {
  ...
  async handleGetRequest(ctx) {
    ...
    switch (requestedData) {
      ...
      case "product":
        ...
        break;
      case "shopifyProduct":
        results = await this._getShopifyProductJSON(
          parseInt(ctx.query.shopifyID)
        );
        break;
  ...
  async _getVariationGroupsJSON(productID) {
    let results = await this._connection.awaitQuery(
      `SELECT * FROM VariationGroups WHERE owningProduct = ?`,
      [productID]
    );
    return JSON.stringify(results);
  }
  ...
  async _getVariationBlockersJSON(variationID) {
    let resultsA = await this._connection.awaitQuery(
      `SELECT excludeVariationB AS exclude FROM VariationBlockers WHERE excludeVariationA = ?`,
      [variationID]
    );
    let resultsB = await this._connection.awaitQuery(
      `SELECT excludeVariationA AS exclude FROM VariationBlockers WHERE excludeVariationB = ?`,
      [variationID]
    );
  ...
  async _postProduct(id, shopifyID, owningUser) {
    if (isNaN(id)) {
      let result = await this._connection.awaitQuery(
        `INSERT INTO Products (shopifyID, owningUser) VALUES (?, ?);`,
        [shopifyID, owningUser]
      );
      return JSON.stringify({ insertedID: result.insertId });
    } else {
      await this._connection.awaitQuery(
        `UPDATE Products SET shopifyID = ?, owningUser = ? WHERE productID = ?;`,
        [shopifyID, owningUser, id]
      );
  ...
  async _postVariationGroup(id, name, owningProduct) {
    if (isNaN(id)) {
      let result = await this._connection.awaitQuery(
        `INSERT INTO VariationGroups (name, owningProduct) VALUES (?, ?);`,
        [name, owningProduct]
      );
      return JSON.stringify({ insertedID: result.insertId });
    } else {
      await this._connection.awaitQuery(
        `UPDATE VariationGroups SET name = ?, owningProduct = ? WHERE groupID = ?;`,
        [name, owningProduct, id]
      );
      return JSON.stringify({});
    }
  }
  ...
  async _postVariation(id, addedCost, name, owningGroup) {
    if (isNaN(id)) {
      let result = await this._connection.awaitQuery(
        `INSERT INTO Variations (addedCost, name, owningGroup) VALUES (?, ?, ?);`,
        [addedCost, name, owningGroup]
      );
      return JSON.stringify({ insertedID: result.insertId });
    } else {
      await this._connection.awaitQuery(
        `UPDATE Variations SET addedCost = ?, name = ?, owningGroup = ? WHERE variationID = ?;`,
        [addedCost, name, owningGroup, id]
      );
      return JSON.stringify({});
    }
  }
  ...
  async _postVariationBlocker(blockerAId, blockerBId) {
    ...
    await this._connection.awaitQuery(
      `INSERT INTO VariationBlockers (excludeVariationA, excludeVariationB) VALUES (?, ?);`,
      [blockerAId, blockerBId]
    );
  ...
  async _deleteVariationGroup(id) {
    if (isNaN(id)) {
      ...
      await this._connection.awaitQuery(
        `DELETE FROM VariationGroups WHERE groupID = ?;`,
        [id]
      );
      return JSON.stringify({ message: "Successfully deleted" });
    }
    ...
    if (isNaN(blockerAID) || isNaN(blockerBID)) {
      ...
      await this._connection.awaitQuery(
        `DELETE FROM VariationBlockers WHERE 
                ...
                (excludeVariationA = ? AND excludeVariationB = ?);`,
        [blockerAID, blockerBID, blockerBID, blockerAID]
      );
      return JSON.stringify({ message: "Successfully deleted" });
    }
  }
